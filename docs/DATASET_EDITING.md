...
```json
{
  ...
  "answer": {
    ...
    "structure": {
      "direct": "进程主要管理资源，线程主要承担执行和调度。",
      "points": [
        {
          "title": "资源与地址空间",
          "content": "进程拥有独立地址空间，同一进程内的线程共享代码、数据和打开文件。"
        },
        {
          "title": "调度与切换",
          "content": "线程是 CPU 调度的基本单位，线程切换通常比进程切换更轻量。"
        }
      ],
      "summary": "一句话记忆：进程管资源，线程管执行。"
    },
...
`answer.structure` 是可选的。填写后，单题训练页会按以下顺序展示：

1. `direct`：先直接回答核心结论；
2. `points`：2–5 个 `{ title, content }` 要点；
3. `summary`：最后用一句话收束。

没有 `structure` 的旧题库仍会完整显示 `spoken`，因此不需要为了兼容性一次性重写所有自定义题目。推荐的写法与标题选择见[结构化口述答案规范](ANSWER_STYLE_GUIDE.md)。
...
- 院校题没有关联学校；
- 复习周期顺序错误；
- 已填写的 `answer.structure.points` 少于 2 个或多于 5 个；
