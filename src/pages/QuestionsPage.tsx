      ...
      <div className="question-browser">
        <details className="filter-panel" open>
          ...
          <div className="filter-content">
            <label>科目<select aria-label="科目" value={filters.subjectId} onChange={(event) => updateFilter("subjectId", event.target.value)}><option value="">全部科目</option>{dataset.subjects.map((subject) => <option key={subject.id} value={subject.id}>{subject.name}</option>)}</select></label>
            <label>优先级<select aria-label="优先级" value={filters.priority} onChange={(event) => updateFilter("priority", event.target.value)}><option value="">全部级别</option><option value="S">S 级</option><option value="A">A 级</option><option value="B">B 级</option></select></label>
            <label>目标院校<select aria-label="目标院校" value={filters.schoolId} onChange={(event) => updateFilter("schoolId", event.target.value)}><option value="">全部院校</option>{dataset.schools.map((school) => <option key={school.id} value={school.id}>{school.name}</option>)}</select></label>
            <label>证据类型<select aria-label="证据类型" value={filters.sourceType} onChange={(event) => updateFilter("sourceType", event.target.value)}><option value="">全部来源</option>{sourceOptions.map(([type, label]) => <option key={type} value={type}>{label}</option>)}</select></label>
            <label>练习状态<select aria-label="练习状态" value={filters.practice} onChange={(event) => updateFilter("practice", event.target.value)}><option value="">全部状态</option><option value="unseen">从未练习</option><option value="practiced">已正式练习</option><option value="today">今日练过</option><option value="due">已到期</option></select></label>
            <label>掌握程度<select aria-label="掌握程度" value={filters.mastery} onChange={(event) => updateFilter("mastery", event.target.value)}><option value="">全部程度</option><option value="0">未练</option><option value="1">红 · 不会</option><option value="2">黄 · 说乱</option><option value="3">绿 · 完整</option><option value="4">熟练 · 能追问</option></select></label>
